...
假设有一个数组arr=[17,23,0,96,17,23],其中所有数据的范围在0~200之间，那么我就创造一个辅助数组help=[],共有201个元素，
...
比如说一个数组[13,17,9,23,27,100]

![avatar](http://code.clouddn.com/02_10_09_study_sort_tongpaixu_02.jpg)
...
1. 确定好最大位数，就遍历几遍
2. 根据每位数进行计数排序
3. 确定好<=0一共有几个，<=1一共有几个。。。。。。。<=9一共有几个
   * 目的主要是在放入桶的时候，确定好每个桶有几个数值，比如说第2桶，他不就是<=2个数-<=1个数
4. 从右→左判断arr原数组中每一个数字个位数数值（因为是遍历，挨个来十位、百位）与每个桶的计数排序下标对应的关系，并且arr这个数字应该放在哪一位？？
   * 比如说100个位数是0，那么就比较计数排序为0的桶<=0一共有几个 ，1个，也就是放在0-0范围，也就是0位置，并将这个数值减去
   * 比如说032个位数是2，那么就比较计数排序为2的桶<=2一共有几个 ，6个，也就是放在0-5范围，也就是5位置，并将这个数值减去
