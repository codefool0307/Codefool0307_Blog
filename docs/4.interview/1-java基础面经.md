...
- [18.java基础-集合map-hashmap的数据结构](#18java基础-集合map-hashmap的数据结构)
  ...
  - [18-6：扰动函数以及作用](#18-6扰动函数以及作用)
  - [18-7：哈希冲突的解决方法](#18-7哈希冲突的解决方法)
  - [18-8：HashMap的put操作](#18-8hashmap的put操作)
  - [18-9：HashMap的get操作](#18-9hashmap的get操作)
...
3. 如果有冲突，则通过key.equals(k)去树或链表中查找对应的entry。其中，树的时间复杂度为O(logn)；链表的时间复杂度为O(n)。
