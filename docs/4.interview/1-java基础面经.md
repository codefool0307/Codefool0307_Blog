 ...
 * @LastEditors: 孙浩然
 * @LastEditTime: 2020-08-18 10:33:09
...
- [9.java基础-运算符-相等问题](#9java基础-运算符-相等问题)
  - [9-1： == 与 equals](#9-1--与-equals)
  - [9-2：为什么要重写hashcode与equals](#9-2为什么要重写hashcode与equals)
  - [9-3：为什么两个对象有相同的hashcode值，它们也不一定是相等的？](#9-3为什么两个对象有相同的hashcode值它们也不一定是相等的)
  - [9-4：说说&和&&的区别。](#9-4说说和的区别)
  - [9-5：用最有效率的方法算出 2 乘以 8 等于几?](#9-5用最有效率的方法算出-2-乘以-8-等于几)
- [10.java基础-数据类型-8种常见类型](#10java基础-数据类型-8种常见类型)
  - [10-1：八种数据类型是什么？-中兴](#10-1八种数据类型是什么-中兴)
  - [10-2：数据类型的范围](#10-2数据类型的范围)
  - [10-3：为什么byte类型是-128~+127](#10-3为什么byte类型是-128127)
  - [10-4：java为什么除了基本数据类型还要有引用数据类型](#10-4java为什么除了基本数据类型还要有引用数据类型)
  - [10-5：String为什么不是基本数据类型-字节](#10-5string为什么不是基本数据类型-字节)
- [11.java基础-数据类型-自动拆装箱](#11java基础-数据类型-自动拆装箱)
  - [11-1：为什么要有自动拆装箱](#11-1为什么要有自动拆装箱)
  - [11-2：Integer缓存机制](#11-2integer缓存机制)
  - [11-3：自动拆装箱的原理](#11-3自动拆装箱的原理)
  - [11-4:自动拆装箱使用场景](#11-4自动拆装箱使用场景)
  - [11-5：自动拆装箱带来的问题](#11-5自动拆装箱带来的问题)
  - [11-6：String转出int型， 判断能不能转？ 如何转？](#11-6string转出int型-判断能不能转-如何转)
  - [11-7：short s1 = 1; s1 = s1 + 1;有什么错? short s1 = 1; s1+=1;有什么错?](#11-7short-s1--1-s1--s1--1有什么错-short-s1--1-s11有什么错)
  - [11-8：int与Integer区别](#11-8int与integer区别)
- [12.java基础-关键字-访问权限关键字](#12java基础-关键字-访问权限关键字)
  - [12-1：访问控制关键字级别](#12-1访问控制关键字级别)
  - [12-2: 通过反射访问private成员和方法，既然能访问为什么要private？](#12-2-通过反射访问private成员和方法既然能访问为什么要private)
- [13.java基础-关键字-static](#13java基础-关键字-static)
  - [13-1:static使用场景](#13-1static使用场景)
- [14.java基础-关键字-final关键字](#14java基础-关键字-final关键字)
  - [14-1 final关键字使用特点](#14-1-final关键字使用特点)
  - [14-2：final, finally, finalize 的区别。](#14-2final-finally-finalize-的区别)
  - [14-3：使用 final 关键字修饰一个变量时，是引用不能变，还是引用的对象不能变？](#14-3使用-final-关键字修饰一个变量时是引用不能变还是引用的对象不能变)
- [15.java基础-关键字-this关键字和super关键字](#15java基础-关键字-this关键字和super关键字)
- [16.java基础-关键字-transient](#16java基础-关键字-transient)
- [17.java基础-集合-hashmap](#17java基础-集合-hashmap)
  - [17-1：hashmap的数据结构](#17-1hashmap的数据结构)
  - [13-2:HashMap中(tab.length - 1) & hash作用](#13-2hashmap中tablength---1--hash作用)
  - [13-3：扰动函数以及作用](#13-3扰动函数以及作用)
  - [如何解决哈希冲突](#如何解决哈希冲突)
  - [13-4：哈希冲突的解决方法](#13-4哈希冲突的解决方法)
  - [13-5：HashMap的put操作](#13-5hashmap的put操作)
  - [13-6:reHash过程](#13-6rehash过程)
  - [13-7：hash函数以及常用方法](#13-7hash函数以及常用方法)
  - [13-8：hashmap树化门槛及作用](#13-8hashmap树化门槛及作用)
  - [13-9：hashmap性能提升作用和好处](#13-9hashmap性能提升作用和好处)
  - [13-10：hashmap的特性](#13-10hashmap的特性)
  - [13-11：java8为什么要对hashmap进行改造，用红黑树替代](#13-11java8为什么要对hashmap进行改造用红黑树替代)
  - [13-12：为什么不把链表全部换为红黑树](#13-12为什么不把链表全部换为红黑树)
  - [13-13：HashMap的扩容机制](#13-13hashmap的扩容机制)
  - [13-14：请解释一下HashMap的参数loadFactor，它的作用是什么？](#13-14请解释一下hashmap的参数loadfactor它的作用是什么)
  - [13-15：HashMap的树化门槛为什么是8](#13-15hashmap的树化门槛为什么是8)
  - [13-13：HashMap的扩容因子为什么是0.75](#13-13hashmap的扩容因子为什么是075)
  - [13-14:为什么默认初始化桶数组大小为16](#13-14为什么默认初始化桶数组大小为16)
  - [13-15:hashMap是否线程安全](#13-15hashmap是否线程安全)
  - [13-16：线程安全的Map](#13-16线程安全的map)
  - [13-17：hashmap为什么是2的秘方次](#13-17hashmap为什么是2的秘方次)
...
- [21.java基础-IO-各种流](#21java基础-io-各种流)
  ...
  - [21-3：BIO\NIO\AIO区别](#21-3bionioaio区别)
  - [21-4：BIO\NIO\AIO使用场景](#21-4bionioaio使用场景)
  - [21-5：linux的5种IO模型](#21-5linux的5种io模型)
  - [21-6:IO多路复用](#21-6io多路复用)
...
- [32.网络编程](#32网络编程)
  - [token的验证流程](#token的验证流程)
  - [token和cookie实现的区别](#token和cookie实现的区别)
  - [Cookie 和 Session 的区别](#cookie-和-session-的区别)
  - [socket连接过程](#socket连接过程)
- [33.JDK新特性](#33jdk新特性)
- [34.操作符号](#34操作符号)
...
## 9-3：为什么两个对象有相同的hashcode值，它们也不一定是相等的？
...
## 11-6：String转出int型， 判断能不能转？ 如何转？

可以转， 得处理异常 Integer.parseInt(s) 主要为 NumberFormatException： 

## 11-7：short s1 = 1; s1 = s1 + 1;有什么错? short s1 = 1; s1+=1;有什么错?
...
## 14-2：final, finally, finalize 的区别。
...
## 14-3：使用 final 关键字修饰一个变量时，是引用不能变，还是引用的对象不能变？

使用 final 关键字修饰一个变量时，是指引用变量不能变，引用变量所指向的对象中的内容还是可以改变的。


# 15.java基础-关键字-this关键字和super关键字
...
2. 扩容后数据存储位置的计算方式也不一样：
   ...
   2. 而在JDK1.8的时候直接用了JDK1.7的时候计算的规律，也就是扩容前的原始位置+扩容的大小值=JDK1.8的计算方式，而不再是
   JDK1.7的那种异或的方法。但是这种方式就相当于只需要判断Hash值的新增参与运算的位是0还是1就直接迅速计算出了扩容后的储
   存方式。

3. JDK1.7的时候使用的是数组+ 单链表的数据结构。HashMap通过key的hashCode经过扰动函数处理过后得到hash值，然后通过(n-1)&hash判断当前元素存放的位置，如果当前位置存在元素的话，就判断该元素与要存入的元素的hash值以及key是否相同，如果相同的话，直接覆盖，不相同就通过拉链法解决冲突。但是在JDK1.8及之后时，使用的是数组+链表+红黑树的数据结构,当链表的深度达到8的时候，也就是默认阈值，就会自动扩容把链表转成红黑树的数据结构，以减少搜索时间。


## 13-2:HashMap中(tab.length - 1) & hash作用
...
2. 元素是无序的，而且顺序会不定时改变
3. key用Set存放，所以想做到key不允许重复，key对应的类需要重写hashCode和equals方法。
4. 底层实现是?链表数组，JDK?8?后又加了红黑树。
5. 实现了?Map?全部的方法

## 13-11：java8为什么要对hashmap进行改造，用红黑树替代

提高效率

## 13-12：为什么不把链表全部换为红黑树
...
## 13-14：请解释一下HashMap的参数loadFactor，它的作用是什么？
...
在JDK1.7的时候没有加入同步锁保护，同时由于JDK1.7数据结构的原因
...
1. 客户端Socket：首先调用Socket类的构造函数，以服务器的指定的IP地址或指定的主机名和指定的端口号为参数，创建一个Socket流。

2. 服务器端Socket：处于等待连接的状态，实时监控网络状态，等待客户端的连接请求。

3. 监听服务器连接：

4. 建立了客户端和服务器端通讯Socket后。就可以使用Socket的方法getInputStream()和getOutputStream()来创建输入/输出流。
