...
- [18.java基础-集合map-hashmap的数据结构](#18java基础-集合map-hashmap的数据结构)
  ...
  - [18-4：为什么在JDK1.7的时候是先进行扩容后进行插入，而在JDK1.8的时候则是先插入后进行扩容的呢？](#18-4为什么在jdk17的时候是先进行扩容后进行插入而在jdk18的时候则是先插入后进行扩容的呢)
  - [18-5：为什么在JDK1.8中进行对HashMap优化的时候，把链表转化为红黑树的阈值是8,而不是7或者不是20呢（面试蘑菇街问过）？](#18-5为什么在jdk18中进行对hashmap优化的时候把链表转化为红黑树的阈值是8而不是7或者不是20呢面试蘑菇街问过)
  - [18-6：为什么不把链表全部换为红黑树](#18-6为什么不把链表全部换为红黑树)
  - [18-7：扰动函数以及作用](#18-7扰动函数以及作用)
  - [18-8：哈希冲突的解决方法](#18-8哈希冲突的解决方法)
  - [18-9：HashMap的put操作](#18-9hashmap的put操作)
  - [18-10:reHash过程](#18-10rehash过程)
...
[范式](https://blog.csdn.net/u010257931/article/details/103143627)


## 18-4：为什么在JDK1.7的时候是先进行扩容后进行插入，而在JDK1.8的时候则是先插入后进行扩容的呢？



## 18-5：为什么在JDK1.8中进行对HashMap优化的时候，把链表转化为红黑树的阈值是8,而不是7或者不是20呢（面试蘑菇街问过）？
