...
3.托管态：也叫做游离态或离线态，它是指持久态对象失去了与session的关联，托管态对象它存在OID,在数据库中有可能存在，也有可能不存在。对于  托管态对象，它发生改变时hibernet不能检测到。
...
1. 面向对象的原则
2. Java类以及类的成员
   属性、方法、构造器、代码块、内部类（匿名内部类）
3. 面向对象的三大特征+抽象性（有的书籍标注不一）
4. 关键字
...
可以使用static关键字，因为如果创建了一个类的多个对象，如果不是static关键字，每个对象都独立的拥有一套类的属性
